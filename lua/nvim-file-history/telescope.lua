...
function M.breadcrumb_picker(opts)
  opts = opts or {}
  ...
  local breadcrumb_history = {}
  for i = 1, math.min(10, #history) do
    if history[i].filepath ~= current_file then
      table.insert(breadcrumb_history, history[i])
    end
  end
  
  if #breadcrumb_history == 0 then
    vim.notify('No recent file breadcrumbs available', vim.log.levels.INFO)
    return
  end
  
  pickers.new(opts, {
    prompt_title = 'Recent Breadcrumbs',
    finder = finders.new_table({
      results = breadcrumb_history,
      entry_maker = function(entry)
        return {
          value = entry,
          display = function(e)
            local idx = nil
            for i, h in ipairs(breadcrumb_history) do
              if h.filepath == e.value.filepath then
                idx = i
                break
              end
            end
            local display_string = string.format('%d. %s (%s)', 
              idx or 0, 
              e.value.relative_path, 
              format_time_ago(e.value.timestamp))
            return display_string
          end,
          ordinal = entry.relative_path,
          path = entry.filepath,
